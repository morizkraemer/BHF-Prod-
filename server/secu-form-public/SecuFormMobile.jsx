...
function SecuFormMobile() {
  ...
  function fetchCurrentEvent() {
    return fetch('/api/current-event')
      ...
      .then(function (data) {
        setEventInfo({
          eventName: (data && data.eventName != null ? String(data.eventName) : '').trim(),
          date: (data && data.date != null ? String(data.date) : '').trim(),
          doorsTime: (data && data.doorsTime != null ? String(data.doorsTime) : '').trim(),
        });
      })
      .catch(function () {
